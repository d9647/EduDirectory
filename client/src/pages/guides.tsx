                          ...
                          <div key={section.subheader} className="mb-4">
                            ...
                            <ul className="space-y-2">
                              {(section.links.length > 5 ? section.links.slice(0, 5) : section.links).map(link => (
                                <li key={link.label}>
                                  <span className="flex items-start gap-2 text-xs text-gray-700 font-normal leading-tight flex-wrap">
                                    <a
                                      href={link.url}
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="flex items-center gap-1 text-blue-700 underline hover:underline whitespace-nowrap"
                                    >
                                      <Send className="h-4 w-4 text-blue-500 mt-0.5 flex-shrink-0" />
                                      <span>{link.label}</span>
                                    </a>
                                    {link.description && (
                                      <span className="ml-1 text-gray-700 font-normal">- {link.description}</span>
                                    )}
                                  </span>
                                </li>
                              ))}
