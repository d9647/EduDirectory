...
async function geocodeAddress(address: string, city?: string, state?: string, zipcode?: string): Promise<{ latitude: number; longitude: number } | null> {
  try {
    ...
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'Educational-Yellow-Pages/1.0 (educational platform)'
      }
    });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/events', async (req, res) => {
    try {
      ...
      // If coordinates are not provided but we have address information, try geocoding
      if ((!latitude || !longitude) && (req.body.venue || req.body.address || req.body.city)) {
        console.log("Attempting to geocode address...");
        const coordinates = await geocodeAddress(
          req.body.address || req.body.venue,
          req.body.city,
          req.body.state,
          req.body.zipcode
        );
  ...
  app.put('/api/admin/edit/:type/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // For events, try geocoding if coordinates are missing but address info exists
      if (type === 'event' && (!updateData.latitude || !updateData.longitude) && 
          (updateData.venue || updateData.address || updateData.city)) {
        console.log("Attempting to geocode updated address...");
        const coordinates = await geocodeAddress(
          updateData.address || updateData.venue,
          updateData.city,
          updateData.state,
          updateData.zipcode
        );
        ...
        if (coordinates) {
          ...
          console.log(`Geocoding successful for update: ${coordinates.latitude}, ${coordinates.longitude}`);
        }
      }
