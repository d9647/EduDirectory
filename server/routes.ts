...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/forum/posts', isAuthenticated, async (req, res) => {
    try {
      const { search, category, sortBy, sortOrder, page = 1, limit = 20 } = req.query;
      ...
      const filters = {
        ...
        offset: (parseInt(page as string) - 1) * (parseInt(limit as string) || 20),
      };

      const result = await storage.getForumPosts(filters);
      res.json(result);
    } catch (error) {
  ...
  app.get('/api/forum/posts/:id/replies', isAuthenticated, async (req, res) => {
    try {
      const postId = parseInt(req.params.id);
  ...
  app.put('/api/forum/replies/:id', isAuthenticated, async (req, res) => {
    try {
  ...
  app.delete('/api/forum/replies/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      await storage.deleteForumReply(replyId, userId);
